...
exports.create_type = async (req, res) => {
  ...
  try {
    const existingUser = await pool.query(
      `SELECT * FROM product_catalog WHERE full_name = $1`,
      [full_name]
    );
    ...
    const userData = await pool.query(
      `INSERT INTO product_catalog(full_name) VALUES($1) returning *`,
      [full_name]
    );
    res.status(201).json({
      error: false,
...
exports.update_type = async (req, res) => {
  ...
  try {
    ...
    updateQuery += ` WHERE product_catalog_id=$${index} returning *`;
    updateValues.push(id);

    const userData = await pool.query(updateQuery, updateValues);

    if (userData.rowCount === 0) {
      res.status(404).json({ error: true, message: "Product catalog not found" });
    } else {
      res.status(200).json({
        error: false,
...
exports.delete_type = async (req, res) => {
  ...
  try {
    const userData = await pool.query(
      "DELETE FROM product_catalog WHERE product_catalog_id=$1 returning *",
      [id]
    );
    if (userData.rowCount === 0) {
      res.status(404).json({ error: true, message: "Product catalog not found" });
...
exports.get_all_types = async (req, res) => {
  try {
    ...
    userData = await pool.query(
      "SELECT * FROM product_catalog ORDER BY product_catalog_id"
    );
...
exports.get_type_by_id = async (req, res) => {
  ...
  try {
    const userData = await pool.query(
      "SELECT * FROM product_catalog WHERE product_catalog_id=$1",
      [id]
    );
    if (userData.rowCount === 0) {
