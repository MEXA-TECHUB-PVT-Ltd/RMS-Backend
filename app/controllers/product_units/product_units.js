...
exports.create_type = async (req, res) => {
  ...
  try {
    const existingUser = await pool.query(
      `SELECT * FROM product_units WHERE full_name = $1`,
      [full_name]
    );
    ...
    if (existingUser.rows.length > 0) {
      return res.status(409).json({
        error: true,
        message: "Product Units already exists",
      });
    }
    const userData = await pool.query(
      `INSERT INTO product_units(full_name,
            symbol,
...
exports.update_type = async (req, res) => {
  ...
  const client = await pool.connect();
  try {
    let updateQuery = "UPDATE product_units SET ";
    let updateValues = [];
    let index = 1;
    ...
    updateQuery += ` WHERE product_units_id=$${index} returning *`;
    updateValues.push(id);

    const userData = await pool.query(updateQuery, updateValues);

    if (userData.rowCount === 0) {
      res.status(404).json({ error: true, message: "Product Units not found" });
    } else {
      res.status(200).json({
        error: false,
...
exports.delete_type = async (req, res) => {
  ...
  try {
    const userData = await pool.query(
      "DELETE FROM product_units WHERE product_units_id=$1 returning *",
      [id]
    );
    if (userData.rowCount === 0) {
      res.status(404).json({ error: true, message: "Product Units not found" });
      ...
      res
        .status(200)
        .json({ error: false, message: "Product Unit deleted successfully" });
...
exports.get_all_types = async (req, res) => {
  try {
    ...
    userData = await pool.query(
        "SELECT * FROM product_units ORDER BY product_units_id"
      );
...
exports.get_type_by_id = async (req, res) => {
  ...
  const client = await pool.connect();
  try {
    const userData = await pool.query("SELECT * FROM product_units WHERE product_units_id=$1", [
      id,
    ]);
    if (userData.rowCount === 0) {
